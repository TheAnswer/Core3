...
//			viewer->frame();
	...
	for (int i=0; i<regions.size(); i++) {
		...
		if(region->sceneData.size() > 0) {
			Vector3 center;
			Sphere largestSphere(center, 0);
			for( int s=0; s<region->regionBounds.size(); s++) {
				Sphere sphere = region->regionBounds.get(s);
				center += sphere.getCenter();
				if(sphere.getRadius() > largestSphere.getRadius()) {
					largestSphere = sphere;
				}
			 }
			
			center = center / region->regionBounds.size();
			
			info("Building region terrain for: " + region->name, true);
			region->sceneData.add(getTerrainMesh(center, ((largestSphere.getCenter() - center).length() + largestSphere.getRadius())*2, 32, 2));
