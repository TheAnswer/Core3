...
Vector<Reference<MeshData*> > SceneObject::getTransformedMeshData() {
	...
	Vector<Reference<MeshData*> > meshes = templateObject->getAppearanceTemplate()->getTransformedMeshData();
...
void SceneObject::loadSnapshotNode(WorldSnapshotNode* node) {
	...
	setPosition(position.getX(), position.getZ(), position.getY());
...
osg::ref_ptr<osg::MatrixTransform> SceneObject::getTransformNode() {
	...
	osg::Quat nodeQuat(direction.getX(), direction.getZ(), -direction.getY(), direction.getW() );
