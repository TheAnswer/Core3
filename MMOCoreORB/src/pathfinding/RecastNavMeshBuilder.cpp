...
void RecastNavMeshBuilder::buildAllTiles(AABB area) {
	...
	for (int y = zStart; y < th; ++y) {
		...
		progress += y - zStart;
		info("Generating tiles: " + String::valueOf(progress * 100 / ((th - zStart) * (tw - xStart))) + "% complete",
			 true);
...
void RecastNavMeshBuilder::dumpOBJ(String filename, Vector <Reference<MeshData*>> data) {
	...
	for (int x = 0; x < data.size(); x++) {
		...
		for (int i = 0; i < tris->size(); i++) {
			MeshTriangle& tri = tris->get(i);
			outputStream.write("f " + String::valueOf(lastIndex + tri.getVerts()[0]) + " " +
							   String::valueOf(lastIndex + tri.getVerts()[1]) + " " +
							   String::valueOf(lastIndex + tri.getVerts()[2]) + "\n");
		}
		lastIndex += verts->size();
