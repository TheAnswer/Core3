...
void RecastNavMeshBuilder::initialize(Zone* zone, Vector<Reference<MeshData*> > meshData, AABB bounds) {
	TerrainManager *terrainManager = zone->getPlanetManager()->getTerrainManager();
	
	info("Building region terrain for: " + name, true);
	Vector3 center = bounds.center();
	meshData.add(getTerrainMesh(center, bounds.extents()[bounds.longestAxis()]*2, terrainManager, 32, 2));
