...
RecastNavMeshBuilder::RecastNavMeshBuilder(AABB meshBounds, Vector<Reference<MeshData*> > meshData, Zone* zone, String name) :
Logger(name),
...
m_meshBounds(Vector3(0, 0, 0), Vector3(0, 0, 0)),
m_tileTriCount(0)
{
	if(zone)
	...
	m_agentMaxClimb = 0.55f;		//buildSettings->agentMaxClimb;
	...
	m_edgeMaxError = 1.3f;			//buildSettings->edgeMaxError;
	m_vertsPerPoly = 6;			//buildSettings->vertsPerPoly;
	m_detailSampleDist = 6;		//buildSettings->detailSampleDist;
	m_detailSampleMaxError = 1.1f;	//buildSettings->detailSampleMaxError;
	m_partitionType = 0;		//buildSettings->partitionType;
	m_navMesh = NULL;
	this->name = name;
	m_initialMeshData = meshData;
	m_currentZone = zone;
	m_ctx = new rcContext();
	m_meshBounds = meshBounds;
}

RecastNavMeshBuilder::RecastNavMeshBuilder(AABB meshBounds, Vector<Reference<MeshData*> > meshData, Zone* zone, String name, dtNavMesh *mesh) :
Logger(name),
m_keepInterResults(false),
m_buildAll(true),
m_totalBuildTimeMs(0),
...
void RecastNavMeshBuilder::initialize(AABB box) {
	
	TerrainManager *tMan = m_currentZone->getPlanetManager()->getTerrainManager();
	
	
	// Square our AABB
	Vector3 center = m_meshBounds.center();
	
	Vector3 extents = m_meshBounds.extents();
	...
	m_meshBounds = AABB(Vector3(center.getX()-longest, center.getY()-longest, center.getZ()-longest),
	...
	// Get Terrain
	info("Building region terrain for: " + name  + (" + m_meshBounds.getMinBound()->toString() + ", " + m_meshBounds.getMaxBound()->toString(), true);
	m_initialMeshData.add(RecastNavMeshBuilder::getTerrainMesh(positionTemp, longest*2, tMan, chunkSize, distanceBetweenHeights));
	
	// Flatten Transforms
	Reference<MeshData*> flattened = flattenMeshData(m_initialMeshData);
	m_initialMeshData.removeAll();
	...
	tMan->getProceduralTerrainAppearance()->getWaterBoundariesInAABB(box, &water);
...
bool RecastNavMeshBuilder::build(AABB bounds)
{
...
void RecastNavMeshBuilder::buildAllTiles(AABB area)
{
	...
	RecastTileBuilder builder(waterTableHeight, 0, 0, lastTileBounds, chunkyMesh);
	builder.changeMesh(m_geom);
	builder.setCellSize(m_cellSize);
	builder.setCellHeight(m_cellHeight);
	builder.setMaxError(m_detailSampleMaxError);
	builder.setTileSize(m_tileSize);
