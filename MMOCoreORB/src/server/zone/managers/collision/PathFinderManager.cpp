...
Vector<WorldCoordinates>* PathFinderManager::findPathFromWorldToWorld(const WorldCoordinates& pointA, const WorldCoordinates& pointB, Zone* zone) {
	...
	Timer t;
	t.start();
	VectorMap<String, ManagedReference<NavMeshRegion*> > regions = zone->getPlanetManager()->getNavMeshes();
	for (int i=0; i<regions.size(); i++) {
		Reference<NavMeshRegion*> region = regions.get(i);
		AABB bbox = region->getBoundingBox();
		const Sphere sphere(bbox.center(), bbox.extents()[bbox.longestAxis()]);
