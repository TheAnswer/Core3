...
NavMeshManager::NavMeshManager() {
    maxConcurrentJobs = ConfigManager::instance()->getMaxNavMeshJobs();
}

void NavMeshManager::enqueueJob(Zone* zone, NavMeshRegion* region, AABB areaToBuild, const RecastSettings& recastConfig) {
    ...
    if (job) {
        ...
        info("Adding area to running job " + name, true);
        return;
    }
    ...
    if (job == NULL) {
        job = new NavMeshJob(region, zone, recastConfig);
        info("Creating new job for " + name, true);
    } else {
        info("Adding area to existing job " + name, true);
    }
...
void NavMeshManager::startJob(Reference<NavMeshJob*> job) {
    ...
    builder->initialize(meshData, bBox, 4.0f);
    meshData.removeAll();
