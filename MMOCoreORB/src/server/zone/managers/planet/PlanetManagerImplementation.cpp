...
void PlanetManagerImplementation::addNavMesh(Reference<NavmeshRegion*> region) {
...
void PlanetManagerImplementation::initialize() {
	...
	for (int i=0; i<navmeshesToBuild.size(); i++) {
		...
		if(region->sceneData.size() > 0) {
			...
			RecastNavMeshBuilder *builder = new RecastNavMeshBuilder(zone, region->name);
			
			Vector3 positionTemp = center;
			
			builder->initialize(region->sceneData, box);
