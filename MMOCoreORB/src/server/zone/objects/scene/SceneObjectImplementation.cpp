...
Matrix4 SceneObjectImplementation::getRecastTransform() {
	Matrix4 rot;
	rot.setRotationMatrix(direction.toMatrix3());
	Matrix4 trans;
	trans.setTranslation(getPositionX(), getPositionY(), getPositionZ());
...
Vector<Reference<MeshData*> > SceneObjectImplementation::getTransformedMeshData(Matrix4* parentTransform) {
	...
	Vector<Reference<MeshData*> > meshes = appearance->getTransformedMeshData(getRecastTransform() * *parentTransform );
