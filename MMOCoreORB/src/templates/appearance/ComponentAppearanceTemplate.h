...
public:
	...
	virtual bool testCollide(const Sphere& testsphere) const {
		for(int i=0; i<meshes.size(); i++) {
			...
			if(mesh->getMeshTemplate()->testCollide(testsphere))
				return true;
		}
	}
	...
	virtual Vector<Reference<MeshData* > > getTransformedMeshData() const {
		...
		for(int i=0; i<meshes.size(); i++) {
			...
			Vector<Reference<MeshData* > > data = mesh->getMeshTemplate()->getTransformedMeshData();
